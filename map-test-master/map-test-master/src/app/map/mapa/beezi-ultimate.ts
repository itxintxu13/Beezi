...
@Component({
  ...
  template: `
    <div class="space-view beezi-ux" style="position:relative;width:100vw;height:100vh;overflow:hidden;">
      ...
      <div class="bee-layer">
        <div *ngFor="let bee of activeBees; trackBy: trackBee" class="bee-follower bee-marker" [style.transform]="'translate(' + bee.x + 'px, ' + bee.y + 'px)" [style.transition]="'transform ' + bee.speed + 'ms linear'" title="Abeja en ruta">🐝</div>
      </div>
...
export class BeeziUltimateComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.route.queryParams.subscribe(params => {
      ...
      if (region) {
        ...
        this.viewMode = 'map';
        setTimeout(() => {
          // Geocodificar la región y centrar el mapa
  ...
  addHotspots() {
    ...
    // Layer de círculos pulsantes
    // (Capa de hotspots pendiente de definir)
  }

  addRoutes() {
    // Rutas de migración entre hotspots
    const routes = [
      { from: [-5.9842, 37.3891], to: [-3.6032, 37.1760] }, // Sevilla -> Jaén
      { from: [-3.6032, 37.1760], to: [-4.4214, 36.7213] }, // Jaén -> Málaga
      { from: [-4.4214, 36.7213], to: [-1.1307, 37.9922] }, // Málaga -> Murcia
  ...
  startBeeAnimation() {
    ...
    const animateBees = () => {
      this.activeBees.forEach(bee => {
        ...
        // Si llegó al final de la ruta, cambiar a la siguiente
        if (bee.progress >= 1) {
          bee.progress = 0;
  ...
  toggleTimelapseMode() {
    ...
    if (this.timelapseMode && this.map) {
      // Simular timelapse con rotación automática
      let bearing = 0;
      const rotateMap = () => {
